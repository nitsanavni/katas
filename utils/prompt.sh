./execsh-gpt.py "
continue the fizzbuzz kata - go up to 15, no need for tests 11-14
...
run the tests as often as possible - just run \`pytest\` - before and after each change
use TDD
always read code before changing it - \`cat\`
