...
void setup(void) { five_dollars = money_create(5, "USD"); }
...
START_TEST(test_money_create_neg) {
  ...
  ck_assert_msg(m == NULL, "NULL should be returned on attempt to create with "
                           "a negative amount");
}
END_TEST
...
Suite *money_suite(void) {
  ...
  tcase_set_timeout(tc_core, 20000);
  ...
  tcase_set_timeout(tc_limits, 20000);
